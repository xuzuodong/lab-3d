<template>
  <div>
    ...
    <div id="mode">
      ...
      <q-input outlined @input="searchKeyWords" v-model="keywords" label="请输入你想搜索的节点" />
    ...
    <div id="info" v-show="selectNodeData.name !== undefined">
      ...
      <q-card :style="{ backgroundColor: selectNodeData.color }" class="node-card">
        <div slot="header" class="clearfix">
          <span>{{ selectNodeData.name }}</span>
          <q-button @click="btnEdit" style="float: right; padding: 3px 0; color: #409efb; font-size: 15px">
            编辑
          </q-button>
        </div>
  ...
  computed: {
    ...
    gDensity() {
      return this.nodes.length <= 1
        ? 0
        : (this.links.length / (this.nodes.length * (this.nodes.length - 1))).toFixed(2)
    },
    gDegree() {
    ...
    searchKeyWords(value) {
      ...
      // 否则判断判断三个元素是否等于name值，等于则显示该值
        ...
        this.svgDom
          ...
          .selectAll('line')
          .attr('class', (d) => {
            if (d.source.properties.name.indexOf(name) >= 0 || d.target.properties.name.indexOf(name) >= 0) {
              return ''
            } else {
              return 'inactive' //隐藏
            }
