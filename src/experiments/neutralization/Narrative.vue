...
export default {
  ...
  async mounted() {
    ...
    const p2_dropAcidType = await this.$dialog({ paragraph: p2 })
    ...
    let explainAgain = await this.$dialog({ paragraph: p6 })
    while (explainAgain == 1) {
      explainAgain = await this.$dialog({ paragraph: p6 })
    }
    ...
    const p21_alkaliType = await this.$dialog({ paragraph: p21 })
    await this.babylon.pullInCamera()
    if (p21_alkaliType == 0) {
      const p21_0 = script.paragraphs.find(p => p.id == '碱溶液-氢氧化钠')
      await this.$dialog({ paragraph: p21_0 })
      this.alkaliType = 'alkali_naoh'
    }
    if (p21_alkaliType == 1) {
      const p21_1 = script.paragraphs.find(p => p.id == '碱溶液-小苏打')
      await this.$dialog({ paragraph: p21_1 })
      this.alkaliType = 'alkali_nahco3'
    }
